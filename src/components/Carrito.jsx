...
export default function Carrito() {
  const {
    ...
    total, cantidadTotal, generarLinkWhatsApp,
  } = useCarrito()
      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            exit={{ x: '100%' }}
            transition={dragX > 0 ? { duration: 0 } : { type: 'spring', damping: 28, stiffness: 300 }}
            style={{ x: dragX > 0 ? dragX : undefined }}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center pb-10">
                  ...
                  <button onClick={() => setIsOpen(false)} className="btn-primary mt-6 text-sm">
                    Ver productos
                    <ArrowRight size={15} />
                  </button>
                </div>
                ...
                items.map((item) => (
                  <div key={item.id} className="flex gap-3 bg-dark-800 rounded-xl p-3 border border-dark-700">
                    <img
                      src={item.imagen}
                      alt={item.nombre}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm font-medium leading-snug line-clamp-2 mb-1">{item.nombre}</p>
                      <p className="text-red-500 font-bold text-sm">{formatPrecio(item.precio * item.cantidad)}</p>
                      <p className="text-gray-600 text-xs">{formatPrecio(item.precio)} c/u</p>
                    </div>
                    <div className="flex flex-col items-end justify-between flex-shrink-0">
                      ...
                      <div className="flex items-center gap-1 bg-dark-700 rounded-lg">
                        <button onClick={() => cambiarCantidad(item.id, -1)} className="p-1.5 text-gray-400 hover:text-white transition-colors">
                          <Minus size={12} />
                        </button>
